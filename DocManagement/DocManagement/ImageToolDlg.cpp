...
ImageToolDlg::ImageToolDlg() :m_WndNum(1)
{
    imgWnd.setupUi(this);
    for (int i = 0; i < 4; i++)
    {
    ...
    connect(pQActShowPsd, &QAction::triggered, this, &ImageToolDlg::showPsdImage);
    connect(pQActTif, &QAction::triggered, this, &ImageToolDlg::saveAsTif);
    connect(pQActBmp, &QAction::triggered, this, &ImageToolDlg::saveAsBmp);
    connect(pQActJpg, &QAction::triggered, this, &ImageToolDlg::saveAsJpg);
    connect(pQActPng, &QAction::triggered, this, &ImageToolDlg::saveAsPng);
    connect(pQActCvt2Psd, &QAction::triggered, this, &ImageToolDlg::saveAsPsd);
    connect(pQActCompare, &QAction::triggered, this, &ImageToolDlg::imageCompare);
    connect(wnds.at(0)->img_wnd, &ImageWidget::newFileSignal, this, &ImageToolDlg::showDetailMessage);

}


void ImageToolDlg::resizeEvent(QResizeEvent * event)
{
    QSize  s = size();
...
void ImageToolDlg::saveImage(Mat & img, QString format)
{
    if (m_WndNum <= 0)return;
...
void ImageToolDlg::setWndNum(int num)
{
    m_WndNum = num;
}

void ImageToolDlg::imageCompare()
{
    int maxWndNum = 4;
    SetWndNum *dlg = new SetWndNum(this);
    connect(dlg, &SetWndNum::okSignals, this, &ImageToolDlg::setWndNum);
    dlg->exec();
    if (m_WndNum <= 0)return;
    QSize s = size();
    m_WndPoints.clear();
...
void ImageToolDlg::showPsdImage()
{
    if (wnds.size() <= 0)return;
    wnds.at(0)->img_wnd->convertToPsd();
}

void ImageToolDlg::showDetailMessage()
{
    if (wnds.size() <= 0)return;
